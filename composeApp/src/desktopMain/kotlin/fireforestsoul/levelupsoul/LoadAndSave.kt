        ...
        if (oldAppVersion != null) {
            if (oldAppVersion > 0) {
                ...
                for (x in 0 until habitsSize) {
                    ...
                    habits[x].typeOfGoalHabits = enumValueOf<TypeOfGoalHabits>(input.getOrNull(4 + plusIndexX).toString())
                    habits[x].needGoal = input.getOrNull(5 + plusIndexX)?.toDouble()!!
                    habits[x].needDays = input.getOrNull(6 + plusIndexX)?.toInt()!!
                    habits[x].startDate = input.getOrNull(7 + plusIndexX)?.let { LocalDate.parse(it) }!!
                    habits[x].lastDate = input.getOrNull(8 + plusIndexX)?.let { LocalDate.parse(it) }!!
                    val habitDaySize = input.getOrNull(9 + plusIndexX)?.toInt()!!
                    habits[x].habitDay = mutableListOf(HabitDay())
                    var plusIndexY = 0
                    for (y in 0 until habitDaySize) {
                        if (y > 0) habits[x].habitDay.add(HabitDay())
                        habits[x].habitDay[y].today = input.getOrNull(10 + plusIndexX + plusIndexY)?.toDouble()!!
                        habits[x].habitDay[y].totalOfAPeriod = input.getOrNull(11 + plusIndexX + plusIndexY)?.toDouble()!!
                        habits[x].habitDay[y].correctly = input.getOrNull(12 + plusIndexX + plusIndexY).toBoolean()
                        plusIndexY += 3
                    }
                    plusIndexX += 8 + plusIndexY
                }
            }
        }
    }
