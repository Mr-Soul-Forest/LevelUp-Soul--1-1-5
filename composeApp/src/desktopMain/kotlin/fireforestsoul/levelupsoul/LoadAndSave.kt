        ...
        if (oldAppVersion != null) {
            if (oldAppVersion > 0) {
                ...
                for (x in 0 until habitsSize) {
                    ...
                    habits[x].startDate = input.getOrNull(plusIndex1000000 + 7 + plusIndexX)?.let { LocalDate.parse(it) }!!
                    habits[x].lastDate = input.getOrNull(plusIndex1000000 + 8 + plusIndexX)?.let { LocalDate.parse(it) }!!
                    ...
                    for (y in 0 until habitDaySize) {
                        ...
                        habits[x].habitDay[y].today = input.getOrNull(plusIndex1000000 + 10 + plusIndexX + plusIndexY)?.toDouble()!!
                        habits[x].habitDay[y].totalOfAPeriod =
                            input.getOrNull(plusIndex1000000 + 11 + plusIndexX + plusIndexY)?.toDouble()!!
                        habits[x].habitDay[y].correctly = input.getOrNull(plusIndex1000000 + 12 + plusIndexX + plusIndexY).toBoolean()
                        plusIndexY += 3
                    }
